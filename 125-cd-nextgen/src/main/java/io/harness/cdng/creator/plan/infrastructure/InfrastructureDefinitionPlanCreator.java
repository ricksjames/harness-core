...
public class InfrastructureDefinitionPlanCreator extends ChildrenPlanCreator<InfrastructureDef>{
  @Inject
  private KryoSerializer kryoSerializer;
  public PlanNode getInfraStepPlanNode(InfrastructureDef pipelineInfrastructure) {
    return PlanNode.builder()
        ...
        .stepParameters(pipelineInfrastructure.getSpec())
  ...
  public LinkedHashMap<String, PlanCreationResponse> createPlanForProvisioner(InfrastructureDef infrastructureDef,
      YamlField infraDefField, String infraStepNodeId, KryoSerializer kryoSerializer) {
    if (!isProvisionerConfigured(infrastructureDef)) {
      return new LinkedHashMap<>();
    }
    validateProvisionerConfig(infrastructureDef.getProvisioner());
  ...
  public boolean isProvisionerConfigured(InfrastructureDef actualInfraConfig) {
    return actualInfraConfig.getProvisioner() != null;
  }
  ...
  @Override
  public Class<InfrastructureDef> getFieldClass() {
    return InfrastructureDef.class;
  }

  @Override
  public Map<String, Set<String>> getSupportedTypes() {
    return Collections.singletonMap(YamlTypes.INFRASTRUCTURE_DEF, Collections.singleton(PlanCreatorUtils.ANY_TYPE));
  }

  @Override
  public LinkedHashMap<String, PlanCreationResponse> createPlanForChildrenNodes(PlanCreationContext ctx, InfrastructureDef config) {
    PlanNode infraStepPlanNode = getInfraStepPlanNode(config);

    if(isProvisionerConfigured(config)){
      createPlanForProvisioner(
              config, ctx.getCurrentField(), infraStepPlanNode.getUuid(), kryoSerializer);
    }

  }

  @Override
  public PlanNode createPlanForParentNode(PlanCreationContext ctx, InfrastructureDef config, List<String> childrenNodeIds) {
    StepParameters stepParameters =
            NGSectionStepParameters.builder().childNodeId(childrenNodeIds.get(0)).logMessage("Infra Definition").build();
    return PlanNode.builder()
