...
import io.harness.watcher.app.DelegateConfig;
...
public class WatcherServiceImplV2 implements WatcherService {
  ...
  @Inject @Named("upgradeExecutor") private ScheduledExecutorService backgroundExecutor;
  ...
  private static final ReentrantReadWriteLock delegateProcessLock = new ReentrantReadWriteLock(true);
  ...
  public void run(boolean upgrade) {
    ...
    if (upgrade) {
      log.info("[New] Upgraded watcher process started with config {}", delegateConfig.toString());
    } else {
      log.info("Starting watcher with config {}", delegateConfig.toString());
    }
  ...
  private void performInitTasks() {
    delegateConfig.setVersion(new VersionInfoManager().getVersionInfo().getVersion());
    try {
  ...
  private void heartbeat() {
    try {
      ...
      heartbeatData.put(WATCHER_VERSION, delegateConfig.getVersion());
  ...
  private void selfDestruct() {
    log.info("Self destructing current delegate");
    log.info("Goodbye");
    System.exit(0);
  }
  ...
  private void startDelegateProcess(String version, String versionFolder, List<String> oldDelegateProcesses,
    ...
    // Start Delegate process and record the process id.
  }
}
