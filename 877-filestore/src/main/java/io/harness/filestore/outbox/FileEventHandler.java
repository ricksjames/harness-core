...
public class FileEventHandler implements OutboxEventHandler {
  ...
  public boolean handle(OutboxEvent outboxEvent) {
    try {
      switch (outboxEvent.getEventType()) {
        case io.harness.filestore.events.FileCreateEvent.FILE_CREATED_EVENT:
          return handleFileCreateEvent(outboxEvent);
        case io.harness.filestore.events.FileUpdateEvent.FILE_UPDATED_EVENT:
          return handleFileUpdateEvent(outboxEvent);
        case io.harness.filestore.events.FileDeleteEvent.FILE_DELETED_EVENT:
  ...
  private boolean handleFileCreateEvent(OutboxEvent outboxEvent) throws IOException {
    io.harness.filestore.events.FileCreateEvent fileCreateEvent =
        objectMapper.readValue(outboxEvent.getEventData(), io.harness.filestore.events.FileCreateEvent.class);
  ...
  private boolean handleFileUpdateEvent(OutboxEvent outboxEvent) throws IOException {
    io.harness.filestore.events.FileUpdateEvent fileUpdateEvent =
        objectMapper.readValue(outboxEvent.getEventData(), io.harness.filestore.events.FileUpdateEvent.class);
  ...
  private boolean handleFileDeleteEvent(OutboxEvent outboxEvent) throws IOException {
    io.harness.filestore.events.FileDeleteEvent fileDeleteEvent =
        objectMapper.readValue(outboxEvent.getEventData(), io.harness.filestore.events.FileDeleteEvent.class);
