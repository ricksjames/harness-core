...
const (
	...
	agentEndpoint = "/agents/link?language=%s&os=%s&arch=%s&framework=%s&version=%s"
)
...
func (c *HTTPClient) DownloadLink(ctx context.Context, language, os, arch, framework, version string) ([]types.DownloadLink, error) {
	path := fmt.Sprintf(agentEndpoint, language, os, arch, framework, version)
