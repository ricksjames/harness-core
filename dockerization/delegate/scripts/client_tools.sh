...
if [[ ( -z "$DISABLE_CLIENT_TOOLS") || ("$DISABLE_CLIENT_TOOLS" = "false") ]]; then
  ...
  && mkdir -m 777 -p client-tools/scm/b1e57f64 \
  && curl -s -L -o client-tools/scm/b1e57f64/scm https://app.harness.io/public/shared/tools/scm/release/b1e57f64/bin/linux/amd64/scm
else
