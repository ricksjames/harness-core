...
if [[ ( -z "$DISABLE_CLIENT_TOOLS") || ("$DISABLE_CLIENT_TOOLS" = "false") ]]; then
  ...
  && mkdir -m 777 -p client-tools/scm/25a60daa \
  && curl -s -L -o client-tools/scm/25a60daa/scm https://app.harness.io/public/shared/tools/scm/release/25a60daa/bin/linux/amd64/scm
else
